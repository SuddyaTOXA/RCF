...
ImageUploaderClass.prototype.getImageHtml = function (imgSrc) {
	...
	if (me.$imageToEdit.length == 1 && me.filePath != null && me.filePath != "") {
		...
		var style = ' width:' + Math.round(me.currentSize.width) + 'px; height:' + Math.round(me.currentSize.height) + 'px;';
		switch (me.alignment) {
			case ALIGN_LEFT:
				break;
			case ALIGN_RIGHT:
		...
		return imgTag
			...
			.replace('{3a}', Math.round(me.currentSize.width))
			.replace('{3b}', Math.round(me.currentSize.height))
			.replace('{4}', me.filePath)
			.replace('{5}', me.croppedFilePath == null ? "" : me.croppedFilePath)
			.replace('{6}', Math.round(me.originalSize.width))
			.replace('{7}', Math.round(me.originalSize.height))
